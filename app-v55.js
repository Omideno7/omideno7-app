...
function getDailyMessage(){ const list=window.dailyMessages||[]; const day=new Date().getDate(); const max=Math.max(1, list.length||1); return list[Math.min(day,max)-1]||list[0]; }
...
function getDeclarationDay(){
 ...
 const list=window.declarationsData||[];
...
function renderNewBirthContent(){
 ...
 ${data.sections.map(sec=>{
   ...
   const videoBlock=sec.id==='videos'?`<div class="video-list"><h3>${sec.videosTitle || t('newBirthVideos')}</h3>${videos.map((v,i)=>{const label=(typeof v[0]==='object'?(v[0][currentLang]||v[0].en):v[0])||(`${t('watchVideo')} ${i+1}`);return `<a class="btn secondary track-link" data-track="new_birth_video_${i+1}" href="${v[1]}" target="_blank" rel="noopener">${label}</a>`;}).join('')}</div>`:'';
   const registerBlock=sec.id==='register'?`<div class="register-after-salvation"><a class="btn primary track-link" data-track="new_birth_register" href="${data.registerUrl}" target="_blank" rel="noopener">${data.registerText}</a></div>`:'';
...
function zagrebDateKey(){
 const now=new Date();
 const zagreb=new Date(now.toLocaleString('en-US',{timeZone:'Europe/Zagreb'}));
 const y=zagreb.getFullYear();
 const m=String(zagreb.getMonth()+1).padStart(2,'0');
 const d=String(zagreb.getDate()).padStart(2,'0');
 return `${y}-${m}-${d}`;
}
...
function readingPlanStartKey(plan){ return `bibleReadingPlanStart_${plan||'one'}_V53`; }
function ensureReadingPlanStart(plan){
 const key=readingPlanStartKey(plan);
 let start=localStorage.getItem(key);
 if(!start){ start=zagrebDateKey(); localStorage.setItem(key,start); }
 return start;
}
function readingPlanDay(plan){
 const schedule=(window.bibleReadingSchedule&&window.bibleReadingSchedule.oneYear)||[];
 const today=zagrebDateKey();
 const start=ensureReadingPlanStart(plan);
 const elapsed=daysBetweenDateKeys(start,today);
 if(schedule.length){
   if(plan==='one') return (elapsed%schedule.length)+1;
   return (elapsed%(schedule.length*2))+1;
 }
 if(plan==='one') return (elapsed%365)+1;
 return (elapsed%730)+1;
}
...
function openReadingPlan(plan){
 ensureReadingPlanStart(plan||'one');
 bibleReaderView='reading-one';
 localStorage.setItem('bibleReaderView',bibleReaderView);
 localStorage.setItem('activeReadingPlanV53',plan||'one');
 renderBibleReader();
}
window.resetReadingPlanV53=function(plan){
 try{ localStorage.setItem(readingPlanStartKey(plan||'one'), zagrebDateKey()); renderBibleReader(); }catch(e){}
};
...
function renderReadingPlan(plan){
 ...
 <button type="button" class="btn light" onclick="window.resetReadingPlanV53&&window.resetReadingPlanV53('${plan}')">↺ ${currentLang==='fa'?'شروع دوباره از روز ۱':'Restart from Day 1'}</button>
...
// V55 exports for Bible verse action patches
try{
  window.getBibleState = getBibleState;
  window.setBibleState = setBibleState;
  window.renderVerseTools = renderVerseTools;
  window.getVerseKeyTerms = getVerseKeyTerms;
  window.bibleRef = bibleRef;
  window.toFaDigits = toFaDigits;
  window.APP_VERSION = 'V55';
}catch(e){ console.warn('V55 export patch warning', e); }
