...
(function(){
  window.APP_VERSION='V58.2';
  function lang(){ return localStorage.getItem('lang') || window.currentLang || 'fa'; }
  const L={
    fa:{apoc:'کتاب‌های اپوکریفا',apocDesc:'جدا از کتاب‌مقدس اصلی؛ برای مطالعه تاریخی و تعلیمی',copy:'کپی آیه',copied:'کپی شد',info:'اطلاعات ترجمه',bibleInfo:'فارسی: ترجمه مژده برای عصر جدید / Today’s Persian Version (FarTPV)، متن بدون تغییر و برای مطالعه رایگان نمایش داده می‌شود. اپوکریفای فارسی: ترجمه داود کاظمیان، کاری از سازمان رازگاه، از فایل‌های ارائه‌شده استخراج شده است. انگلیسی: King James Version with Apocrypha (KJVA). کرواتی: فعلاً همان متن قبلی اپ حفظ شده است.'},
  ...
  function setVersion(){
    document.querySelectorAll('.app-version,.version,.footer-version').forEach(el=>{ if(/V\d|Version|نسخه|Verzija/i.test(el.textContent||'')) el.textContent='App Version: V58.2'; });
    const foot=document.querySelector('footer,.footer'); if(foot && !document.querySelector('.v57-version-badge')){ const s=document.createElement('div'); s.className='v57-version-badge'; s.textContent='App Version: V58.2'; foot.appendChild(s); }
  }
