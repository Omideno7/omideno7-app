 ...
 <div class="card reading-plan-card">
   ...
   <div class="reading-plan-list">
     ${items.map(item=>{
       ...
       return `<button type="button" class="reading-plan-item" data-plan-book="${item.bookId}" data-plan-chapter="${item.chapter}" onclick="openPlanChapter(this.dataset.planBook,this.dataset.planChapter)">
...
function renderBibleReader(){
 ...
 if(bibleReaderView==='home'){
   ...
   <div class="bible-section-grid">
     ${sections.map(sec=>`<button type="button" class="bible-section-card " data-bible-section="${sec.id}" onclick="setBibleSection(this.dataset.bibleSection)">
   ...
   <div class="bible-reading-plans card">
     ...
     <div class="bible-section-grid">
       <button type="button" class="bible-section-card reading-plan-btn" data-reading-plan="one" onclick="openReadingPlan('one')"><span class="bible-section-icon">365</span><strong>${t('oneYearPlan')}</strong><small>${t('oneYearPlanDesc')}</small></button>
     </div>
   </div>`;
   bindBibleReaderEvents(root);
   return;
 ...
 if(bibleReaderView==='books'){
   ...
   <div class="bible-book-grid">
     ${books.map(b=>`<button type="button" class="bible-book-card" data-bible-book="${b.id}" onclick="openBibleBook(this.dataset.bibleBook)"><strong>${bibleBookName(b)}</strong><small>${currentLang==='fa'?toFaDigits(b.chapters):b.chapters} ${t('chapters')}</small></button>`).join('')}
   </div>`;
   bindBibleReaderEvents(root);
