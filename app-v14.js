...
function openBibleHome(){
 try{
  ...
  activeBibleSection=localStorage.getItem('bibleSection') || 'new';
...
function readingPlanDay(plan){
 ...
 if(schedule.length){
   if(plan==='one') return ((dayOfYearForCroatia()-1)%schedule.length)+1;
   return (((daysSinceBaseForCroatia()-1)%(schedule.length*2))+1);
 }
...
function readingPlanItems(plan){
 ...
 if(schedule.length){
   ...
   const twoDay=readingPlanDay('two');
   const baseIndex=Math.floor((twoDay-1)/2)%schedule.length;
   const refs=((schedule[baseIndex]||{}).refs||[]);
   if(refs.length<=1) return refs;
   const cut=Math.ceil(refs.length/2);
   return (twoDay%2===1?refs.slice(0,cut):refs.slice(cut)) || refs;
 }
...
function renderReadingPlan(plan){
 ...
 <div class="card reading-plan-card">
   ...
   <p><strong>${t('planDay')}:</strong> ${currentLang==='fa'?toFaDigits(day):day}</p><p class="reading-plan-source-note">${t('readingPlanPdfSource')}</p>
