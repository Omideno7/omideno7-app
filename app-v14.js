...
let dailySpeechQueue = [];
let dailySpeechIndex = 0;
...
function speakDailyChunks(chunks){
 if(!chunks.length)return;
 if(dailySpeechIndex>=chunks.length)return;
 const utterance=new SpeechSynthesisUtterance(chunks[dailySpeechIndex]);
 utterance.lang=getSpeechLang();
 utterance.rate=currentLang==='fa'?0.78:0.88;
 utterance.pitch=1;
 utterance.volume=1;
 const voices=window.speechSynthesis.getVoices ? window.speechSynthesis.getVoices() : [];
 const preferred=voices.find(v=>v.lang && v.lang.toLowerCase().startsWith(getSpeechLang().slice(0,2).toLowerCase()));
 if(preferred) utterance.voice=preferred;
 utterance.onend=()=>{dailySpeechIndex++; setTimeout(()=>speakDailyChunks(chunks),80);};
 utterance.onerror=()=>{dailySpeechIndex++; setTimeout(()=>speakDailyChunks(chunks),80);};
 window.speechSynthesis.speak(utterance);
}
function speakDailyWord(){
 if(!('speechSynthesis' in window) || !('SpeechSynthesisUtterance' in window)){
  alert(t('audioUnsupported'));
  return;
 }
 try{
  window.speechSynthesis.cancel();
  dailySpeechIndex=0;
  dailySpeechQueue=splitSpeechText(buildDailySpeechText());
  speakDailyChunks(dailySpeechQueue);
 }catch(e){
  alert(t('audioUnsupported'));
 }
}
function stopDailySpeech(){
 ...
 dailySpeechQueue=[]; dailySpeechIndex=0;
}
...
function renderDaily(){
 ...
 document.querySelectorAll('[data-daily-card]').forEach(card=>{
   ...
   ${declarationHtml}
   ${prayerHtml}
   <div class="daily-audio-controls">
     <button class="btn gold daily-audio-btn" type="button" onclick="speakDailyWord()">🔊 ${t('listenDaily')}</button>
     <button class="btn secondary daily-audio-btn" type="button" onclick="stopDailySpeech()">■ ${t('stopReading')}</button>
   </div>`;
 });
}

function renderPlans(){ const root=document.getElementById('plansContent'); if(!root||!window.teachingPlans)return; if(!selectedPlanKey || !window.teachingPlans[selectedPlanKey]){ let html=`<div class="hero-card"><h1>${t('selectPlan')}</h1><p>${t('plansDesc')}</p></div>`; Object.keys(window.teachingPlans).forEach(key=>{ const p=window.teachingPlans[key][currentLang]||window.teachingPlans[key].en; html+=`<div class="plan-list-card" data-plan-key="${key}"><h3>${p.title}</h3><p>${p.subtitle}</p><div class="plan-progress"><div style="width:10%"></div></div><button class="btn primary">${t('startPlan')}</button></div>`; }); root.innerHTML=html; root.querySelectorAll('[data-plan-key]').forEach(card=>card.addEventListener('click',()=>{selectedPlanKey=card.dataset.planKey; selectedPlanDay=parseInt(localStorage.getItem('planDay_'+selectedPlanKey)||'1',10); localStorage.setItem('selectedPlanKey',selectedPlanKey); renderPlans();})); return; } const p=window.teachingPlans[selectedPlanKey][currentLang]||window.teachingPlans[selectedPlanKey].en; const d=p.days[Math.max(1,Math.min(selectedPlanDay, p.days.length))-1]; const progress=Math.round((d.day/p.days.length)*100); const img=''; root.innerHTML=`<div class="hero-card"><h1>${p.title}</h1><p><strong>${p.subtitle}</strong></p><p>${p.intro}</p><div class="plan-progress"><div style="width:${progress}%"></div></div></div><div class="plan-day-card">${img}<span class="day-label">${t('day')} ${d.day}</span><h3>${d.title}</h3><div class="plan-section-title">${t('scriptures')}</div>${d.scriptures.map(s=>`<details class="plan-scripture-expand"><summary>${loc(s.ref)}</summary><p>${loc(s.text)}</p></details>`).join('')}<div class="plan-section-title">${t('devotional')}</div><p>${d.devotional}</p><div class="plan-section-title">${t('practicalStep')}</div><p>${d.step}</p><div class="plan-section-title">${t('prayer')}</div><p>${d.prayer}</p><div class="plan-section-title">${t('declaration')}</div><p><strong>${d.declaration}</strong></p><div class="plan-day-nav"><button class="btn light" id="backToPlans">${t('backToPlans')}</button><button class="btn primary" id="nextPlanDay">${d.day>=p.days.length?t('finishPlan'):t('continueToNextDay')}</button></div></div>`; document.getElementById('backToPlans').addEventListener('click',()=>{selectedPlanKey=null; localStorage.removeItem('selectedPlanKey'); renderPlans();}); document.getElementById('nextPlanDay').addEventListener('click',()=>{ if(selectedPlanDay < p.days.length){ selectedPlanDay++; localStorage.setItem('planDay_'+selectedPlanKey, selectedPlanDay); renderPlans(); window.scrollTo({top:0,behavior:'smooth'}); } else { selectedPlanKey=null; localStorage.removeItem('selectedPlanKey'); renderPlans(); } }); }
...
function currentChapterVerses(){
 ...
 return (((data.chapters||{})[selectedBibleBook]||{})[String(selectedBibleChapter)]||{})[currentLang] || [];
}
function renderBibleReader(){
 const root=document.getElementById('bibleReaderContent');
 if(!root || !window.bibleReaderData)return;
 const data=window.bibleReaderData;
 const books=data.books||[];
 const book=books.find(b=>b.id===selectedBibleBook)||books[0];
 if(book && book.id!==selectedBibleBook)selectedBibleBook=book.id;
 const maxCh=book?book.chapters:1;
 if(selectedBibleChapter>maxCh)selectedBibleChapter=1;
 const verses=currentChapterVerses();
 const note=(data.meta&&data.meta.note)?(data.meta.note[currentLang]||data.meta.note.en):'';
 root.innerHTML=`<div class="section-title"><h2>${t('bibleReaderTitle')}</h2></div>
 <div class="hero-card bible-reader-hero"><h1>${t('bibleReaderTitle')}</h1><p>${t('bibleReaderDesc')}</p><div class="bible-starter-note"><strong>${t('bibleDataStarter')}</strong><br>${note||t('bibleDataStarterDesc')}</div></div>
 <div class="bible-reader-tools">
   <label>${t('selectBook')}<select id="bibleBookSelect">${books.map(b=>`<option value="${b.id}" ${b.id===selectedBibleBook?'selected':''}>${bibleBookName(b)}</option>`).join('')}</select></label>
 ...
 <div class="bible-chapter-card">
   ...
   <div class="bible-verses">${verses.length?verses.map(v=>renderBibleVerse(book.id,selectedBibleChapter,v)).join(''):`<p class="empty-note">${t('noBibleText')}</p>`}</div>
 </div>`;
 document.getElementById('bibleBookSelect')?.addEventListener('change',e=>{selectedBibleBook=e.target.value;selectedBibleChapter=1;localStorage.setItem('bibleBook',selectedBibleBook);localStorage.setItem('bibleChapter','1');renderBibleReader();});
 document.getElementById('bibleChapterSelect')?.addEventListener('change',e=>{selectedBibleChapter=parseInt(e.target.value,10);localStorage.setItem('bibleChapter',String(selectedBibleChapter));renderBibleReader();});
...
function renderBibleVerse(bookId, chapter, verseObj){
 ...
 return `<div class="${cls.join(' ')}" data-bible-verse="${verseObj.v}">
  <div class="bible-verse-line"><span class="verse-number">${currentLang==='fa'?toFaDigits(verseObj.v):verseObj.v}</span><span class="verse-text">${verseObj.t}</span></div>
  <div class="bible-verse-actions">
    <button onclick="toggleBibleState('${bookId}',${chapter},${verseObj.v},'bookmark')">${st.bookmark?'★':'☆'} ${t('bookmark')}</button>
    <button onclick="toggleBibleState('${bookId}',${chapter},${verseObj.v},'read')">✓ ${t('markRead')}</button>
    <button onclick="toggleBibleState('${bookId}',${chapter},${verseObj.v},'bold')">B ${t('boldVerse')}</button>
    <button onclick="cycleBibleHighlight('${bookId}',${chapter},${verseObj.v})">▣ ${t('highlight')}</button>
    <button onclick="openBibleNote('${bookId}',${chapter},${verseObj.v})">✎ ${t('noteVerse')}</button>
  </div>
...
function renderBibleSearch(q){
 ...
 Object.keys(data.chapters||{}).forEach(bookId=>{
  Object.keys(data.chapters[bookId]||{}).forEach(ch=>{
   const arr=(data.chapters[bookId][ch]||{})[currentLang]||[];
   arr.forEach(v=>{
 ...
 root.innerHTML=`<div class="card"><h3>${t('bibleSearchResults')}</h3>${results.length?results.slice(0,25).map(r=>`<button class="bible-search-result" onclick="selectedBibleBook='${r.bookId}';selectedBibleChapter=${r.ch};localStorage.setItem('bibleBook',selectedBibleBook);localStorage.setItem('bibleChapter',String(selectedBibleChapter));renderBibleReader();">${bibleRef(r.bookId,r.ch,r.v.v)} — ${r.v.t}</button>`).join(''):`<p>${t('noResults')}</p>`}</div>`;
}


document.addEventListener('DOMContentLoaded',()=>{ document.querySelectorAll('.lang-toggle button').forEach(b=>b.addEventListener('click',()=>setLang(b.dataset.lang))); document.querySelectorAll('.nav-btn').forEach(b=>b.addEventListener('click',()=>showPage(b.dataset.page))); document.querySelectorAll('[data-open]').forEach(b=>b.addEventListener('click',()=>showPage(b.dataset.open))); document.querySelectorAll('.enable-notifications').forEach(b=>b.addEventListener('click',enableNotifications)); loadAnalyticsIfConfigured(); setLang(currentLang); });
