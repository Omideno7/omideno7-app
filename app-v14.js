...
function currentChapterVerses(){
 const data=window.bibleReaderData||{};
 const lang=(currentLang==='fa'||currentLang==='en'||currentLang==='hr')?currentLang:'en';
 const chapterData=(((data.chapters||{})[selectedBibleBook]||{})[String(selectedBibleChapter)]||{});
 return (chapterData[lang]&&chapterData[lang].length?chapterData[lang]:null) || (chapterData.en&&chapterData.en.length?chapterData.en:null) || chapterData.fa || chapterData.hr || [];
}
...
function renderBibleReader(){
 ...
 if(bibleReaderView==='home'){
   ...
   <div class="bible-section-grid">
     ${sections.map(sec=>`<button class="bible-section-card ${sec.id==='apocrypha'?'apocrypha-card':''}" onclick="setBibleSection('${sec.id}')">
 ...
 if(bibleReaderView==='apocrypha'){
   ...
   <button class="btn ghost bible-back-btn" onclick="setBibleReaderView('home')">← ${t('back')}</button>
   ...
   <div class="bible-book-grid">
     ${apBooks.map(b=>{
       ...
       return `<button class="bible-book-card ${isPending?'pending-book':''}" ${isPending?'disabled':''} onclick="openApocryphaBook('${b.id}')"><strong>${apBookName(b)}</strong><small>${isPending?t('pendingText'):(isExternal?t('externalText'):(loaded?((currentLang==='fa'?toFaDigits(loaded):loaded)+' '+t('selectChapter')):t('englishAvailable')))}</small></button>`;
     }).join('')}
   </div>`;
 ...
 if(bibleReaderView==='books'){
   ...
   <div class="bible-book-grid">
     ${books.map(b=>`<button class="bible-book-card" onclick="selectedBibleBook='${b.id}';selectedBibleChapter=1;localStorage.setItem('bibleBook',selectedBibleBook);localStorage.setItem('bibleChapter','1');setBibleReaderView('reader');"><strong>${bibleBookName(b)}</strong><small>${currentLang==='fa'?toFaDigits(b.chapters):b.chapters} ${t('selectChapter')}</small></button>`).join('')}
   </div>`;
   document.getElementById('bibleSearchInput')?.addEventListener('input',e=>renderBibleSearch(e.target.value));
   return;
 }

 let book=books.find(b=>b.id===selectedBibleBook)||books[0];
 if(!book){setBibleReaderView('home');return;}
 ...
 <button class="btn ghost bible-back-btn" onclick="setBibleReaderView('books')">← ${t('back')}</button>
 ...
 document.getElementById('bibleBookSelect')?.addEventListener('change',e=>{selectedBibleBook=e.target.value;selectedBibleChapter=1;activeBibleVerseRef=null;localStorage.setItem('bibleBook',selectedBibleBook);localStorage.setItem('bibleChapter','1');renderBibleReader();});
 document.getElementById('bibleChapterSelect')?.addEventListener('change',e=>{selectedBibleChapter=parseInt(e.target.value,10);activeBibleVerseRef=null;localStorage.setItem('bibleChapter',String(selectedBibleChapter));renderBibleReader();});
 document.getElementById('bibleSearchInput')?.addEventListener('input',e=>renderBibleSearch(e.target.value));
}
...
function renderBibleSearch(q){
 ...
 const results=[];
 const data=window.bibleReaderData||{};
 const allowedBooks=new Set(activeBibleSection==='apocrypha'?(data.apocryphaBooks||[]).map(b=>b.id):((activeBibleSection==='old'||activeBibleSection==='new')?booksForActiveSection().map(b=>b.id):(data.books||[]).map(b=>b.id)));
 Object.keys(data.chapters||{}).forEach(bookId=>{
  if(!allowedBooks.has(bookId))return;
  Object.keys(data.chapters[bookId]||{}).forEach(ch=>{
   const arr=(data.chapters[bookId][ch]||{})[currentLang] || (data.chapters[bookId][ch]||{}).fa || (data.chapters[bookId][ch]||{}).en || [];
   arr.forEach(v=>{
 ...
 root.innerHTML=`<div class="card"><h3>${t('bibleSearchResults')}</h3>${results.length?results.slice(0,40).map(r=>`<button class="bible-search-result" onclick="selectedBibleBook='${r.bookId}';activeBibleSection=((window.bibleReaderData.books||[]).find(b=>b.id==='${r.bookId}')||{}).section||activeBibleSection;selectedBibleChapter=${r.ch};localStorage.setItem('bibleSection',activeBibleSection);localStorage.setItem('bibleBook',selectedBibleBook);localStorage.setItem('bibleChapter',String(selectedBibleChapter));setBibleReaderView('reader');">${bibleRef(r.bookId,r.ch,r.v.v)} — ${r.v.t}</button>`).join(''):`<p>${t('noResults')}</p>`}</div>`;
}
