...
/* OMIDNO7 AUDIO COLLAPSE + ONESIGNAL LANGUAGE TAG OVERRIDE 2026-05-31 */
(function(){
  function al(){ return window.currentLang || localStorage.getItem('lang') || document.documentElement.lang || 'fa'; }

  function ensureOneSignalLanguageTag(){
    const lang=al();
    try{
      if(window.OneSignalDeferred){
        window.OneSignalDeferred.push(async function(OneSignal){
          try{
            if(OneSignal.User && OneSignal.User.addTag){ await OneSignal.User.addTag('language', lang); }
            else if(OneSignal.sendTag){ OneSignal.sendTag('language', lang); }
          }catch(e){ console.warn('OneSignal language tag failed', e); }
        });
      } else if(window.OneSignal && window.OneSignal.User && window.OneSignal.User.addTag){
        window.OneSignal.User.addTag('language', lang);
      } else if(window.OneSignal && window.OneSignal.sendTag){
        window.OneSignal.sendTag('language', lang);
      }
    }catch(e){}
  }

  const oldSetLang = window.setLang;
  if(typeof oldSetLang === 'function' && !window.__omidLangTagPatch){
    window.__omidLangTagPatch = true;
    window.setLang = function(lang){
      oldSetLang(lang);
      setTimeout(ensureOneSignalLanguageTag, 800);
    };
  }
  document.addEventListener('DOMContentLoaded',()=>setTimeout(ensureOneSignalLanguageTag,1200));

  window.renderSpiritualAudioList=function(cat){
    window.currentAudioCategory=cat||'morning';
    const list=document.getElementById('audioMessagesList'); if(!list)return;
    const source=(typeof spiritualAudioItems!=='undefined' && Array.isArray(spiritualAudioItems)) ? spiritualAudioItems : (window.spiritualAudioItems||[]);
    const items=source.filter(x=>x.category===window.currentAudioCategory);
    const empty = al()==='fa'?'فعلاً پیامی در این دسته‌بندی ثبت نشده است.':al()==='hr'?'Trenutno nema poruka u ovoj kategoriji.':'No messages in this category yet.';
    list.innerHTML = items.length ? items.map(item=>{
      const note=localStorage.getItem('audio_note_'+item.id)||'';
      const fav=localStorage.getItem('audio_favorite_'+item.id)==='1';
      const detailsId='audio-details-'+item.id;
      return `<details class="card audio-message-card audio-collapsed-card" id="audio-card-${item.id}">
        <summary class="audio-summary">
          <span class="audio-tag">${item.tag||''}</span>
          <strong>${item.title}</strong>
          <small>${al()==='fa'?'برای باز کردن و گوش دادن کلیک کنید':al()==='hr'?'Kliknite za slušanje':'Click to open and listen'}</small>
        </summary>
        <div id="${detailsId}" class="audio-details-body">
          <p>${item.desc||''}</p>
          <audio class="spiritual-audio-el" id="audio-player-${item.id}" preload="metadata" controls controlsList="nodownload noplaybackrate" src="${item.src}"></audio>
          <div class="audio-control-bar">
            <button type="button" onclick="playAudio('${item.id}')">▶︎ / ⏸</button>
            <button type="button" onclick="seekAudio('${item.id}',-15)">-۱۵ ثانیه</button>
            <button type="button" onclick="seekAudio('${item.id}',15)">+۱۵ ثانیه</button>
          ...
          <div class="audio-speed-row">
            <span>${al()==='fa'?'سرعت:':al()==='hr'?'Brzina:':'Speed:'}</span>
            <button type="button" data-speed-for="${item.id}" data-rate="1" class="active" onclick="setAudioSpeed('${item.id}',1)">1x</button>
            <button type="button" data-speed-for="${item.id}" data-rate="1.25" onclick="setAudioSpeed('${item.id}',1.25)">1.25x</button>
            <button type="button" data-speed-for="${item.id}" data-rate="1.5" onclick="setAudioSpeed('${item.id}',1.5)">1.5x</button>
            <button type="button" data-speed-for="${item.id}" data-rate="2" onclick="setAudioSpeed('${item.id}',2)">2x</button>
          ...
          <div class="audio-action-row">
            <button type="button" class="btn light" onclick="toggleAudioNote('${item.id}')">${al()==='fa'?'یادداشت':al()==='hr'?'Bilješka':'Note'}</button>
            <button type="button" class="btn light" onclick="toggleAudioFavorite('${item.id}')">${fav?'★':'☆'} ${al()==='fa'?'ستاره‌دار':al()==='hr'?'Označi':'Favorite'}</button>
            <button type="button" class="btn gold" onclick="shareAudioMessage('${item.id}')">${al()==='fa'?'اشتراک‌گذاری لینک':al()==='hr'?'Podijeli poveznicu':'Share link'}</button>
          </div>
          <div class="audio-note-box" id="audio-note-box-${item.id}" hidden>
            <textarea id="audio-note-${item.id}" maxlength="12000" placeholder="${al()==='fa'?'یادداشت خود را از این پیام صوتی بنویسید...':al()==='hr'?'Napišite svoju bilješku...':'Write your note...'}">${note}</textarea>
            <div class="note-tools"><button class="btn primary" onclick="saveAudioNote('${item.id}')">${al()==='fa'?'ذخیره یادداشت':al()==='hr'?'Spremi bilješku':'Save note'}</button><span id="audio-note-status-${item.id}" class="status"></span></div>
          </div>
        </div>
      </details>`;
    }).join('') : `<div class="card"><p>${empty}</p></div>`;

    // Pause audio automatically when a card is closed.
    list.querySelectorAll('details.audio-message-card').forEach(d=>{
