...
function booksForActiveSection(){
 ...
 if(activeBibleSection==='apocrypha') return data.apocryphaBooks||[];
...
function setBibleSection(sectionId){
 ...
 if(sectionId!=='apocrypha' && books[0]){
 ...
 bibleReaderView=sectionId==='apocrypha'?'apocrypha':'books';
...
function currentChapterVerses(){
 ...
 if(chapterData[lang] && chapterData[lang].length) return chapterData[lang];
 ...
 if(chapterData.en && chapterData.en.length) return chapterData.en;
 if(chapterData.fa && chapterData.fa.length) return chapterData.fa;
 return chapterData.hr || [];
}
...
function readingPlanDay(plan){
 if(plan==='one') return ((dayOfYearForCroatia()-1)%365)+1;
 return (((daysSinceBaseForCroatia()-1)%730)+1);
}
function readingPlanItems(plan){
 const seq=getBibleChapterSequence();
 const totalDays=plan==='one'?365:730;
...
function renderReadingPlan(plan){
 ...
 <button type="button" class="btn ghost bible-back-btn" data-bible-view="home">← ${t('backToPlans')}</button>
 <div class="card reading-plan-card">
   ...
   <div class="reading-plan-list">
     ${items.map(item=>{
       ...
       return `<button type="button" class="reading-plan-item" data-plan-book="${item.bookId}" data-plan-chapter="${item.chapter}">
 ...
 <div class="reading-plan-full-text">
   ${items.map(item=>{
     ...
     const lang=getBibleTextLang();
     const verses=(chapterObj[lang]&&chapterObj[lang].length?chapterObj[lang]:(chapterObj.fa&&chapterObj.fa.length?chapterObj.fa:chapterObj.en||chapterObj.hr||[]));
...
function renderBibleReader(){
 ...
 if(bibleReaderView==='reading-one' || bibleReaderView==='reading-two'){
   const plan=bibleReaderView==='reading-one'?'one':'two';
 ...
 if(bibleReaderView==='home'){
   ...
   <div class="bible-section-grid">
     ${sections.map(sec=>`<button type="button" class="bible-section-card ${sec.id==='apocrypha'?'apocrypha-card':''}" data-bible-section="${sec.id}">
       <span class="bible-section-icon">${sec.id==='old'?'◈':sec.id==='new'?'✦':'◇'}</span>
       <strong>${sectionLabel(sec)}</strong>
       <small>${sectionDesc(sec)}</small>
     </button>`).join('')}
   </div>
   <div class="bible-reading-plans card">
     <h3>${t('readingPlans')}</h3>
     <div class="bible-section-grid">
       <button type="button" class="bible-section-card reading-plan-btn" data-reading-plan="one"><span class="bible-section-icon">365</span><strong>${t('oneYearPlan')}</strong><small>${t('oneYearPlanDesc')}</small></button>
       <button type="button" class="bible-section-card reading-plan-btn" data-reading-plan="two"><span class="bible-section-icon">730</span><strong>${t('twoYearPlan')}</strong><small>${t('twoYearPlanDesc')}</small></button>
     </div>
   </div>
   <div class="card audio-bible-placeholder"><h3>${t('audioBibleTitle')}</h3><p>${t('audioBibleDesc')}</p></div>
   <div class="bible-starter-note"><strong>${t('bibleDataStarter')}</strong><br>${note||t('bibleDataStarterDesc')}</div>`;
   bindBibleReaderEvents(root);
   return;
 }

 if(bibleReaderView==='apocrypha'){
   const apBooks=data.apocryphaBooks||[];
   root.innerHTML=`<div class="section-title"><h2>${t('apocrypha')}</h2></div>
   <button type="button" class="btn ghost bible-back-btn" data-bible-view="home">← ${t('back')}</button>
   <div class="card apocrypha-notice"><h3>${t('apocryphaIntroTitle')}</h3>${getApocryphaIntro().split('\n').filter(Boolean).map(p=>`<p>${p}</p>`).join('')}</div>
   ...
   <div class="bible-book-grid">
     ${apBooks.map(b=>{
       const loaded=apocryphaChapterCount(b.id);
       const isExternal=b.source==='external';
       const isPending=b.source==='pending';
       return `<button type="button" class="bible-book-card ${isPending?'pending-book':''}" ${isPending?'disabled':''} data-apocrypha-book="${b.id}"><strong>${apBookName(b)}</strong><small>${isPending?t('pendingText'):(isExternal?t('externalText'):(loaded?((currentLang==='fa'?toFaDigits(loaded):loaded)+' '+t('selectChapter')):t('englishAvailable')))}</small></button>`;
     }).join('')}
   </div>`;
   bindBibleReaderEvents(root);
   document.getElementById('bibleSearchInput')?.addEventListener('input',async e=>{await loadApocryphaEnglish(); renderBibleSearch(e.target.value);});
   return;
 }
 ...
 if(bibleReaderView==='books'){
   ...
   <div class="bible-book-grid">
     ${books.map(b=>`<button type="button" class="bible-book-card" data-bible-book="${b.id}"><strong>${bibleBookName(b)}</strong><small>${currentLang==='fa'?toFaDigits(b.chapters):b.chapters} ${t('chapters')}</small></button>`).join('')}
   </div>`;
   bindBibleReaderEvents(root);
 ...
 if(activeBibleSection==='apocrypha'){
   book=(data.apocryphaBooks||[]).find(b=>b.id===selectedBibleBook) || book;
 }
 if(!book){
 ...
 const maxCh=activeBibleSection==='apocrypha'?Math.max(1,apocryphaChapterCount(selectedBibleBook)||book.chapters||1):(book.chapters||1);
 ...
 <button type="button" class="btn ghost bible-back-btn" data-bible-view="${activeBibleSection==='apocrypha'?'apocrypha':'books'}">← ${t('back')}</button>
...
function renderBibleVerse(bookId, chapter, verseObj){
 ...
 return `<div class="${cls.join(' ')}" data-bible-verse="${verseObj.v}">
  <button type="button" class="bible-verse-line clean-verse-line" data-verse-toggle="1" data-book="${bookId}" data-chapter="${chapter}" data-verse="${verseObj.v}">
