...
(function(){
  'use strict';
  var VERSION='V63.88 Final Admin + Registration Flow Stability Fix';
  ...
  function loadScript(src){return new Promise(function(resolve,reject){if(window.supabase&&window.supabase.createClient){resolve();return;}var done=false;var timer=setTimeout(function(){if(done)return;done=true;reject(new Error('Supabase SDK load timeout'));},9000);function ok(){if(done)return;done=true;clearTimeout(timer);resolve();}function bad(){if(done)return;done=true;clearTimeout(timer);reject(new Error('Supabase SDK load failed'));}var s=document.querySelector('script[data-om7-supabase-sdk][src="'+src+'"]');if(s){s.addEventListener('load',ok,{once:true});s.addEventListener('error',bad,{once:true});setTimeout(function(){if(window.supabase&&window.supabase.createClient)ok();},300);return;}s=document.createElement('script');s.src=src;s.async=true;s.dataset.om7SupabaseSdk='1';s.onload=ok;s.onerror=bad;document.head.appendChild(s);});}
  async function client(){if(sb&&sb.from&&sb.auth)return sb;var names=['omideno7Supabase','schoolSupabase','supabaseClient','om7Supabase','OMIDENO7_SUPABASE_CLIENT','__supabase'];for(var i=0;i<names.length;i++){var c=window[names[i]];if(c&&c.from&&c.auth){sb=c;return sb;}}if(window.OMIDENO7_GET_SUPABASE_CLIENT){sb=await window.OMIDENO7_GET_SUPABASE_CLIENT();if(sb&&sb.from&&sb.auth)return sb;}if(window.OMIDENO7_SUPABASE_READY){sb=await window.OMIDENO7_SUPABASE_READY;if(sb&&sb.from&&sb.auth)return sb;}if(!window.supabase||!window.supabase.createClient){var urls=['https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2','https://unpkg.com/@supabase/supabase-js@2/dist/umd/supabase.js','https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/dist/umd/supabase.min.js'];var lastErr=null;for(var u=0;u<urls.length;u++){try{await loadScript(urls[u]);if(window.supabase&&window.supabase.createClient)break;}catch(e){lastErr=e;}}if(!window.supabase||!window.supabase.createClient)throw(lastErr||new Error('Supabase SDK could not be loaded. Please check internet connection and reload the app.'));}sb=window.supabase.createClient(SUPABASE_URL,SUPABASE_KEY,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});window.omideno7Supabase=sb;window.schoolSupabase=sb;window.supabaseClient=sb;return sb;}
