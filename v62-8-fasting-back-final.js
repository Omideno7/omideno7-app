/* V62.8 Fasting Back Button Final Fix
...
(function(){
  ...
  function isFastingSelectedOrVisible(){
    ...
    const selected2 = (()=>{ try { return (localStorage.getItem('selectedPlanKey') || '').toLowerCase(); } catch(e) { return ''; } })();
    if(selected === 'fastingjourney' || selected === 'fasting' || selected2 === 'fastingjourney' || selected2 === 'fasting') return true;
  ...
  function installDirectFastingBackGuards(){
    const force = function(ev){
      if(!activePlansPage() || !isFastingSelectedOrVisible()) return;
      ev.preventDefault();
      ev.stopPropagation();
      if(ev.stopImmediatePropagation) ev.stopImmediatePropagation();
    ...
    // Replace the black/global back button behavior only while inside fasting journey.
    const gb = document.getElementById('globalBackV50');
    if(gb && !gb.__v628FastBack){
      gb.__v628FastBack = true;
      gb.addEventListener('click', force, true);
    }
    // Replace visible fasting internal Back buttons so they cannot go to fasting home first.
    document.querySelectorAll('#plans [data-fast-view="home"], #plans button, #plans a').forEach(function(el){
      if(isBackControl(el) && !el.__v628FastBack){
        el.__v628FastBack = true;
        el.addEventListener('click', force, true);
      }
    });
  }

  document.addEventListener('DOMContentLoaded', function(){
    installDirectFastingBackGuards();
    setInterval(installDirectFastingBackGuards, 700);
  });
