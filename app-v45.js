...
(function(){
 ...
 .fasting-pillars span{background:#fff;border:1px solid var(--line);border-radius:16px;padding:12px;text-align:center;font-weight:900;color:var(--blue);}
 ...
 .health-note{background:#FFF8E8;border:1px solid #E9D69A;color:#5B4100;padding:12px;border-radius:16px;margin:12px 0;line-height:1.8;}
...
const fastingTexts = {
 fa:{
  ...
  shortTeaching:"در مسیحیت، ارزش روزه به اجبار، نمایش مذهبی یا گرسنگی نیست؛ بلکه به قلبی است که در حضور خداوند قرار می‌گیرد. ما روزه نمی‌گیریم تا خدا را مجبور کنیم کاری انجام دهد؛ روزه می‌گیریم تا توجه خود را از جسم به روح منتقل کنیم، زمان بیشتری در دعا و کلام بگذرانیم، حساسیت روحانی خود را نسبت به صدای خدا تقویت کنیم و قلب خود را در حضور خداوند متمرکز سازیم. عیسی در متی ۶:۱۶-۱۸ فرمود وقتی روزه می‌گیرید، مانند ریاکاران نباشید. این نشان می‌دهد روزه بخشی از زندگی روحانی ایماندار است، اما نه به شکل نمایشی و مذهبی.",
  healthTitle:"توجه مهم سلامتی",
  health:"اگر بیماری خاص، بارداری، دیابت، مشکل قلبی، داروی خاص یا شرایط پزشکی دارید، قبل از روزه غذایی با پزشک مشورت کنید. روزه مسیحی نباید به بدن شما آسیب بزند. می‌توانید به جای روزه غذایی، زمانی را برای دعا، کلام، پرهیز از رسانه‌ها یا تمرکز روحانی اختصاص دهید.",
  ...
  nonFood:["روزه شبکه‌های اجتماعی","روزه تلویزیون و سرگرمی","روزه صحبت‌های منفی","روزه عادت‌های مزاحم","روزه زمان برای دعا و کلام"],
  topics:["نزدیکی بیشتر با خداوند","شنیدن صدای خدا","هدایت برای تصمیم مهم","رشد روحانی","تقدیس و پاکی","دعا برای خانواده","دعا برای کلیسا","دعا برای شفا","دعا برای خدمت","دعا برای نجات جان‌ها","تقویت ایمان","غلبه بر ترس","غلبه بر وسوسه","دریافت مکاشفه از کلام خدا","آمادگی برای خدمت","روزه شکرگزاری","روزه برای بیداری روحانی","روزه و دعا"],
 ...
 en:{
  ...
  shortTeaching:"In Christianity, the value of fasting is not compulsion, religious display, or hunger; it is a heart positioned before the Lord. We do not fast to force God to act. We fast to shift attention from the flesh to the spirit, spend more time in prayer and the Word, strengthen spiritual sensitivity to God’s voice, and focus the heart in His presence. In Matthew 6:16-18, Jesus said when you fast, do not be like the hypocrites. This shows fasting is part of a believer’s spiritual life, but not as religious performance.",
  healthTitle:"Important Health Notice",
  health:"If you have a medical condition, pregnancy, diabetes, heart problems, medication, or any health concern, consult a doctor before a food fast. Christian fasting should not harm your body. You may choose a non-food fast, such as focused prayer, Scripture, media abstinence, or spiritual concentration.",
 ...
 hr:{
  ...
  shortTeaching:"U kršćanstvu vrijednost posta nije prisila, religiozno pokazivanje ili glad, nego srce koje stoji pred Gospodinom. Ne postimo da bismo prisilili Boga da nešto učini. Postimo da preusmjerimo pažnju s tijela na duh, provedemo više vremena u molitvi i Riječi, ojačamo duhovnu osjetljivost na Božji glas i usmjerimo srce u Njegovoj prisutnosti. U Mateju 6,16-18 Isus kaže: kad postite, ne budite kao licemjeri. To pokazuje da je post dio duhovnog života vjernika, ali ne kao religiozna predstava.",
  healthTitle:"Važna zdravstvena napomena",
  health:"Ako imate bolest, trudnoću, dijabetes, srčane probleme, posebne lijekove ili bilo koje zdravstveno stanje, prije posta od hrane posavjetujte se s liječnikom. Kršćanski post ne smije štetiti vašem tijelu. Možete izabrati post koji nije vezan uz hranu, kao što su molitva, Božja riječ, odricanje od medija ili duhovni fokus.",
...
function renderFastingHome(){
 ...
 <div class="fasting-pillars"><span>📖 ${currentLang==='fa'?'کلام':currentLang==='hr'?'Riječ':'Word'}</span><span>🙏 ${currentLang==='fa'?'دعا':currentLang==='hr'?'Molitva':'Prayer'}</span><span>🕊️ ${currentLang==='fa'?'تعمق':currentLang==='hr'?'Razmišljanje':'Meditation'}</span><span>✍️ ${currentLang==='fa'?'مکاشفه':currentLang==='hr'?'Otkrivenje':'Revelation'}</span></div>
 <details class="card fasting-teaching"><summary>${x.shortTeachingTitle}</summary><p>${x.shortTeaching}</p><div class="health-note"><strong>${x.healthTitle}</strong><br>${x.health}</div></details>
 <div class="fasting-menu">
  ...
  <button class="plan-list-card" onclick="fastingSetView('journal')"><h3>${x.journal}</h3><p>${x.revelationToday}</p></button>
  <button class="plan-list-card" onclick="fastingSetView('stats')"><h3>${x.stats}</h3><p>${x.personalStatsNote}</p></button>
  <button class="plan-list-card" onclick="fastingSetView('teaching')"><h3>${x.teaching}</h3><p>${x.shortTeachingTitle}</p></button>
 </div>`;
}
function renderFastingStart(){
 ...
 <form id="fastingStartForm" class="card fasting-form">
   <h3>${x.typeLabel}</h3>
   <select name="type">${x.types.map(v=>`<option>${v}</option>`).join('')}</select>
   <div class="health-note"><strong>${x.healthTitle}</strong><br>${x.health}</div>
   <details><summary>${currentLang==='fa'?'گزینه‌های روزه غیرغذایی':currentLang==='hr'?'Mogućnosti posta bez hrane':'Non-food fast options'}</summary><ul>${x.nonFood.map(v=>`<li>${v}</li>`).join('')}</ul></details>
   <h3>${x.schedule}</h3>
   ...
   <label class="check-row"><input name="reminder" type="checkbox"> ${x.fields.reminder}</label>
   <h3>${x.topic}</h3>
   <select name="topic">${x.topics.map(v=>`<option>${v}</option>`).join('')}</select>
...
function saveFastingStartForm(form){
 ...
 const fast={id:Date.now(), type:data.get('type'), startDate, endDate, startTime:data.get('startTime'), endTime:data.get('endTime'), reminder:!!data.get('reminder'), topic:data.get('customTopic')||data.get('topic'), totalDays:fastingTotalDays(startDate,endDate), createdAt:new Date().toISOString()};
...
function renderFastingTeaching(){
 ...
 <div class="card"><h3>${x.healthTitle}</h3><p>${x.health}</p></div>
