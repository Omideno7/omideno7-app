...
function saveFastingStartForm(form){
 ...
 const fast={id:Date.now(), type:data.get('type'), nonFood:data.getAll('nonFood'), startDate, endDate, startTime:data.get('startTime'), endTime:data.get('endTime'), topic:data.get('customTopic')||data.get('topic'), totalDays:fastingTotalDays(startDate,endDate), createdAt:new Date().toISOString()};
...
function renderFastingActive(){
 ...
 return `<button class="btn light" onclick="fastingSetView('home')">← ${x.back}</button>`+store.active.map(f=>{
  ...
  return `<div class="card active-fasting-card">
   <h2>${x.activeFast}</h2><p><strong>${x.topic}:</strong> ${f.topic}</p><p><strong>${x.fields.type}:</strong> ${f.type}</p>${f.nonFood&&f.nonFood.length?`<p><strong>${currentLang==='fa'?'گزینه‌های انتخابی':currentLang==='hr'?'Odabrane mogućnosti':'Selected options'}:</strong> ${f.nonFood.join('، ')}</p>`:''}<p><strong>${x.currentDay}:</strong> ${currentLang==='fa'?toFaDigits(day):day} / ${currentLang==='fa'?toFaDigits(f.totalDays):f.totalDays}</p><div class="plan-progress"><div style="width:${progress}%"></div></div>
